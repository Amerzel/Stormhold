...
func _spawn_encounters() -> void:
	...
	for enc in GameData.encounters:
		...
		var x: float = float(pos.get("x", 0.0)) * tile_size
		var y: float = float(pos.get("y", 0.0)) * tile_size
		...
		for ref in entity_refs:
			var entity_id: String = String(ref).replace("ea:", "")
			var entity := GameData.get_entity(entity_id)
			if entity.get("category") == "enemy":
				_spawn_enemy(encounter_node, entity, float(enc.get("data", {}).get("radius", 4.0)))
...
func _spawn_enemy(parent: Node2D, entity: Dictionary, radius: float) -> void:
	...
	var tile_size := float(GameData.get_tile_size())
	...
	if enemy.has_method("initialize_spawn"):
		enemy.initialize_spawn(parent.global_position, radius)
